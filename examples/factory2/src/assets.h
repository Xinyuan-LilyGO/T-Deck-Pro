#ifndef MY_ASSETS_H
#define MY_ASSETS_H

#ifdef __cplusplus
extern "C" {
#endif

#include "lvgl.h"

// image
LV_IMG_DECLARE(img_lora)
LV_IMG_DECLARE(img_loratest)
LV_IMG_DECLARE(img_loratest2)
LV_IMG_DECLARE(img_SD)
LV_IMG_DECLARE(img_about)
LV_IMG_DECLARE(img_setting)
LV_IMG_DECLARE(img_setting1)
LV_IMG_DECLARE(img_setting2)
LV_IMG_DECLARE(img_other)
LV_IMG_DECLARE(img_GPS)
LV_IMG_DECLARE(img_gpstsst)
LV_IMG_DECLARE(img_batt)
LV_IMG_DECLARE(img_batterytest)
LV_IMG_DECLARE(img_test)
LV_IMG_DECLARE(img_test_custom)
LV_IMG_DECLARE(img_test_custom2)
LV_IMG_DECLARE(img_test_custom3)
LV_IMG_DECLARE(img_test_custom3_alt)
LV_IMG_DECLARE(img_wifi)
LV_IMG_DECLARE(img_wifi2)
LV_IMG_DECLARE(img_4k)
LV_IMG_DECLARE(img_A7682E)
LV_IMG_DECLARE(img_calltest)
LV_IMG_DECLARE(img_PCM5102)
LV_IMG_DECLARE(img_touch)
LV_IMG_DECLARE(img_inputtest)
LV_IMG_DECLARE(img_start)
LV_IMG_DECLARE(img_motor)
LV_IMG_DECLARE(img_motortest)
LV_IMG_DECLARE(img_shutdowntest)

// font
LV_FONT_DECLARE(Font_Mono_Bold_14)
LV_FONT_DECLARE(Font_Mono_Bold_15)
LV_FONT_DECLARE(Font_Mono_Bold_16)
LV_FONT_DECLARE(Font_Mono_Bold_17)
LV_FONT_DECLARE(Font_Mono_Bold_18)
LV_FONT_DECLARE(Font_Mono_Bold_19)
LV_FONT_DECLARE(Font_Mono_Bold_20)

#ifdef __cplusplus
} /*extern "C"*/
#endif

#endif
